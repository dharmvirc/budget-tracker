...
| Concern | Choice | Notes |
|---|---|---|
| Backend unit tests | JUnit 5 + Mockito + AssertJ | Service-layer business logic; repositories mocked |
| Backend DB/slice tests | `@DataJpaTest` + TestContainers (PostgreSQL) | Real PostgreSQL; validates recursive CTEs, indexes, aggregate queries |
| Backend controller tests | `@WebMvcTest` + MockMvc | HTTP layer, serialisation, auth enforcement, validation errors |
| Backend integration tests | `@SpringBootTest` + TestContainers | Full-stack critical flows; no mocks |
| Frontend unit tests | Vitest | Pure logic: conversions, rollup formulas, Zod schemas |
| Frontend component tests | Vitest + React Testing Library + MSW | Component behaviour with mocked API responses |
| UI automation | Playwright | End-to-end golden-path and edge-case flows in headless Chromium |
| CI | GitHub Actions | Unit + integration tests on every push; Playwright on PR to main |
...
└── e2e/                         ← Playwright UI automation
    ...
    ├── fixtures/                # shared page objects + test data helpers
    │   ├── auth.fixture.ts
    │   └── transaction.fixture.ts
    └── specs/
        ├── auth.spec.ts
        ├── transactions.spec.ts
        ├── itemised-purchase.spec.ts
        ├── budget-alerts.spec.ts
        ├── recurring.spec.ts
        ├── reports.spec.ts
        ├── csv-import.spec.ts
        ├── ocr-receipt.spec.ts
        ├── email-import.spec.ts
        ├── reconciliation.spec.ts
        ├── settings-hierarchy.spec.ts
        ├── person-management.spec.ts
        └── tenant-isolation.spec.ts
```

Backend test layout mirrors the source tree:
```
backend/src/test/java/com/budgettracker/
├── domain/
│   ├── person/
│   │   ├── PersonServiceTest.java          # unit
│   │   └── PersonRepositoryTest.java       # @DataJpaTest + TC
│   ├── transaction/
│   │   ├── TransactionServiceTest.java     # unit
│   │   ├── TransactionControllerTest.java  # @WebMvcTest
│   │   └── TransactionRepositoryTest.java  # @DataJpaTest + TC
│   ├── budget/
│   │   ├── BudgetServiceTest.java
│   │   └── BudgetAlertServiceTest.java
│   ├── recurring/
│   │   └── RecurringTransactionJobTest.java
│   ├── category/
│   │   └── CategoryRepositoryTest.java     # recursive CTE tests
│   ├── product/
│   │   └── ProductRepositoryTest.java      # full-text search tests
│   ├── unit/
│   │   └── UnitConversionServiceTest.java
│   ├── emailimport/
│   │   ├── EmailPollerJobTest.java
│   │   ├── BlinkitEmailParserTest.java
│   │   ├── AmazonEmailParserTest.java
│   │   ├── ZeptoEmailParserTest.java
│   │   ├── SwiggyEmailParserTest.java
│   │   ├── ZomatoEmailParserTest.java
│   │   └── FlipkartEmailParserTest.java
│   ├── ocr/
│   │   └── OcrServiceTest.java
│   └── reconciliation/
│       └── CcStatementParserTest.java
└── integration/
    ├── HouseholdRegistrationFlowTest.java
    ├── TransactionBudgetAlertFlowTest.java
    ├── RecurringTransactionFlowTest.java
    ├── TenantIsolationTest.java
    └── EmailImportFlowTest.java
```
...
- [ ] Configure Vitest + React Testing Library + MSW for frontend; confirm `npm test` runs
- [ ] Configure Playwright (`playwright.config.ts`) + fixtures directory; confirm `npx playwright test` runs (zero specs is fine)
- [ ] Configure separate `integrationTest` Gradle source set; confirm TestContainers pulls `postgres:16` and Flyway migrations run in CI

**Verification**: `./gradlew bootRun` + `npm run dev` → `/api/health` returns 200, React app loads. `./gradlew test` green. `npm test` green.
...
- [ ] **Tests**: `PersonServiceTest`, `PersonRepositoryTest`, `AuthControllerTest`, `HouseholdRegistrationFlowTest`, `TenantIsolationTest`; Playwright `auth.spec.ts` + `person-management.spec.ts`
...
- [ ] **Tests**: `CategoryRepositoryTest` (recursive CTE, delete guard, depth performance), `UnitConversionServiceTest`; Playwright `settings-hierarchy.spec.ts`
...
- [ ] **Tests**: controller tests for Accounts and Stores (CRUD, deactivation role check, 409 on in-use delete)
...
- [ ] **Tests**: `TransactionServiceTest` (rollup, alert trigger, category inheritance, spent_for default), `TransactionControllerTest`, `TransactionRepositoryTest`; frontend `TransactionForm.test.tsx`, `NotificationBell.test.tsx`, `rollupValidation.test.ts`; Playwright `transactions.spec.ts`
...
- [ ] **Tests**: `UnitConversionServiceTest` (all system conversions, chained, custom), `ProductRepositoryTest` (FTS, auto-create, price history), `unitConversion.test.ts`; frontend `LineItemRow.test.tsx`; Playwright `itemised-purchase.spec.ts`
...
- [ ] **Tests**: `BudgetAlertServiceTest` (threshold, no double-fire, recursive roll-up), `TransactionBudgetAlertFlowTest` (integration); frontend `BudgetProgressBar.test.tsx`; Playwright `budget-alerts.spec.ts`
...
- [ ] **Tests**: `RecurringTransactionJobTest` (INCOME/EXPENSE handling, variable amount, idempotency), `RecurringTransactionFlowTest` (integration, restart + re-run); Playwright `recurring.spec.ts`
...
- [ ] **Tests**: `TransactionRepositoryTest` aggregate queries (monthly breakdown, income vs expense trend, per-account roll-up, recursive category roll-up); Playwright `reports.spec.ts`
...
- [ ] **Tests**: `OcrServiceTest` (pre-processing smoke, heuristic parser, graceful empty result), `CcStatementParserTest` (entry count, date/amount, Tesseract fallback); Playwright `csv-import.spec.ts`, `ocr-receipt.spec.ts`, `reconciliation.spec.ts`
...
- [ ] **Tests**: `EmailPollerJobTest` (known/unknown sender, dedup, parser crash isolation), all platform parser tests with saved email fixtures in `src/test/resources/emails/`, `EmailImportFlowTest` (integration with mock IMAP); Playwright `email-import.spec.ts`
...
Tests are written **alongside** each phase — not after. Each phase's definition of done includes its test tasks. The sections below define what to test and how for each domain area.

### 9.1 Backend — Unit Tests (JUnit 5 + Mockito + AssertJ)

**`PersonServiceTest`**
- `createWithUser()` atomically creates both rows; rolls back if either insert fails
- Deactivating a user sets `status = INACTIVE`; person row is untouched
- Deleting a non-login person blocked if referenced by any transaction
- `createWithUser()` for SuperAdmin: person has `household_id = null`

**`UnitConversionServiceTest`**
- All 13 system conversions produce correct factors (kg↔g, g↔mg, L↔mL, each↔dozen, m↔cm, cm↔mm, m↔ft, ft↔in, yd↔ft, and their inverses)
- Chained conversion (kg → mg) via intermediate unit
- Custom conversion overrides do not affect system conversions
- `convert()` throws on unknown unit pair

**`TransactionServiceTest`**
- Rollup passes: `Σ(line item totals) + gst_paid + delivery_charges - discount = amount`
- Rollup fails → 422 unless `amount_override = true`
- Budget alert check is called exactly once per transaction save
- Category inheritance: line item with no category gets parent transaction's category in reports
- `spent_for_person_id` defaults to the household's `is_whole_household` person when not supplied

**`BudgetAlertServiceTest`**
- Alert fires at exact threshold (80%, 100%)
- Alert does not double-fire for same threshold + period
- Spend in a subcategory rolls up to parent budget (recursive CTE)
- No alert fires if no budget defined for that category

**`RecurringTransactionJobTest`**
- INCOME rule → CONFIRMED transaction created, `next_due_date` advanced
- EXPENSE rule → PENDING_REVIEW transaction created + notification
- Variable amount rule → amount left null
- Job run twice same day: second run is a no-op (unique index guard)
- Rule with `is_active = false` is skipped

**`EmailPollerJobTest`**
- Known sender pattern → dispatches to correct parser
- Unknown sender → inserts PENDING_REVIEW row with `extracted_data = null`
- Duplicate `platform_order_id` → skipped, no new row inserted
- Parser throws → row inserted with error status, job continues (does not crash)

**Per-platform parser tests** (`BlinkitEmailParserTest`, `AmazonEmailParserTest`, etc.)
- Each test loads a saved raw email file from `src/test/resources/emails/`
- Asserts: order date, order ID, line item count, item names, prices, delivery charge, total
- Separate test for edge cases: order with no line items, zero delivery charge, multiple GST rates

**`OcrServiceTest`**
- Pre-processing pipeline produces a non-null image (smoke test)
- Heuristic parser extracts amount, date, merchant from a known test receipt
- Parser returns empty `OcrResult` (not exception) for unreadable input

**`CcStatementParserTest`**
- Parses a sample PDF statement: entry count, dates, amounts match expected
- Falls back to Tesseract path when PDF text extraction returns empty

---

### 9.2 Backend — Repository / DB Tests (`@DataJpaTest` + TestContainers)

**`CategoryRepositoryTest`**
- Subtree query returns all descendants at any depth
- Subtree query returns empty set for a leaf node
- In-use guard: delete blocked when a transaction references a descendant category
- Recursive CTE performance: 500 categories, 5 levels deep — query under 100ms

**`TransactionRepositoryTest`**
- Monthly category aggregate returns correct sum for the current month
- Income vs expense trend groups correctly by month across a 12-month range
- `spent_by_person_id` filter returns only that person's transactions
- Account roll-up: all Credit Card account transactions summed correctly via account type tree

**`ProductRepositoryTest`**
- Full-text search: `"ground nut"` matches "Ground Nut Oil"
- Partial match: `"nut"` returns multiple results
- New product auto-created when line item name has no match
- Price history records accumulate across multiple transactions

---

### 9.3 Backend — Controller Tests (`@WebMvcTest` + MockMvc)

One controller test class per controller. Standard cases to cover for every endpoint:

| Scenario | Assert |
|---|---|
| Valid request, authenticated Member | 200/201, correct response shape |
| Valid request, unauthenticated | 401 |
| Admin-only endpoint called by Member | 403 |
| Invalid request body (missing required field) | 422 with field-level error |
| Resource not found | 404 |
| Delete of in-use entity | 409 |

**`TransactionControllerTest`** — additionally:
- File upload endpoint accepts `multipart/form-data`; returns receipt path in response
- List endpoint: pagination params `page` + `size` respected; filter by `type`, `status`, date range

**`AuthControllerTest`**
- Login with correct credentials → 200 + session cookie set
- Login with wrong password → 401
- Logout → session cookie cleared
- `GET /me` after logout → 401
- `must_change_password = true` → `/me` returns flag; other endpoints return 403 until changed

---

### 9.4 Backend — Integration Tests (`@SpringBootTest` + TestContainers)

**`HouseholdRegistrationFlowTest`**
1. `POST /api/v1/households/register` → 201, status PENDING
2. SuperAdmin login → sees pending registration notification
3. SuperAdmin approves → household APPROVED, "Whole household" person seeded
4. Registrant logs in → 200, role = ADMIN
5. Admin creates a Member → person row + user row both created

**`TransactionBudgetAlertFlowTest`**
1. Create budget: Groceries, ₹5,000/month, thresholds 80% + 100%
2. Enter transactions totalling ₹3,999 → no alert
3. Enter ₹101 more → 80% threshold notification created
4. Enter ₹901 more → 100% threshold notification created
5. Run again → no duplicate notifications for same threshold + period

**`RecurringTransactionFlowTest`**
1. Create monthly EXPENSE recurring rule
2. Trigger job → PENDING_REVIEW transaction created
3. Trigger job again same day → transaction count unchanged (idempotency)
4. Restart app, trigger job → still no duplicate
5. Confirm the pending transaction → status = CONFIRMED, budget alert check fires

**`TenantIsolationTest`**
1. Two households each with one transaction
2. Household A member calls `GET /api/v1/transactions` → sees only A's transaction
3. Household A member calls `GET /api/v1/transactions/{idFromHouseholdB}` → 404
4. Household A member calls `PUT /api/v1/transactions/{idFromHouseholdB}` → 404

**`EmailImportFlowTest`**
1. Seed platform sender pattern for Blinkit
2. Mock IMAP server returns one Blinkit order email
3. Trigger `EmailPollerJob`
4. `GET /api/v1/email-import/queue` → one PENDING_REVIEW entry with correct extracted data
5. Confirm → transaction created, queue entry status = CONFIRMED
6. Trigger poller again → same email not re-imported (Message-ID dedup)

---

### 9.5 Frontend — Unit Tests (Vitest)

**`unitConversion.test.ts`**
- All 13 system conversion factors correct to 5 decimal places
- Inverse conversion: `convert(1, 'ft', 'm')` = 1 / 3.28084
- Unknown unit pair throws descriptive error

**`rollupValidation.test.ts`**
- `Σ(line items) + gstPaid + delivery - discount = amount` → valid
- Off by ₹1 without `amount_override` → invalid
- Off by ₹1 with `amount_override = true` → valid
- Empty line items: transaction amount alone is always valid

**`zodSchemas.test.ts`**
- Transaction schema rejects negative amount
- Transaction schema rejects future date (more than 1 day ahead)
- Line item schema requires `unitPrice > 0` and `quantity != 0`
- Password schema rejects strings shorter than 8 characters

---

### 9.6 Frontend — Component Tests (Vitest + React Testing Library + MSW)

**`TransactionForm.test.tsx`**
- Renders all required fields; optional fields hidden until toggled
- Submit with empty amount → inline validation error, no API call
- Submit valid income → calls `POST /api/v1/transactions` with correct payload
- Line item added → running total updates
- Rollup mismatch banner shown when sum ≠ amount (and no override)

**`HierarchyTree.test.tsx`**
- Renders root nodes and expandable children
- "Add child" adds a node under the selected parent
- Rename inline: double-click → input → blur → `PUT` called
- Delete leaf node → confirmation dialog → `DELETE` called
- Delete in-use node → API returns 409 → error toast shown, node not removed

**`LineItemRow.test.tsx`**
- Typing in product name fires debounced search after 300ms
- Selecting a product auto-fills default unit and category
- Changing quantity updates the row total display
- Unit different from product default → conversion hint shown

**`BudgetProgressBar.test.tsx`**
- 0–79%: green bar
- 80–99%: amber bar
- 100%+: red bar, "Over budget" label
...
### 9.7 UI Automation — Playwright

All specs run against the full running stack (backend + PostgreSQL + frontend). Playwright config sets `baseURL` from an env var so CI points at a test instance.

**`auth.spec.ts`**
- Happy path: register → SuperAdmin approves → login → see dashboard → logout
- Login with wrong password → error message shown, no redirect
- Session expires → revisit page → redirected to login
- SuperAdmin login → must-change-password screen → change → sees admin panel

**`person-management.spec.ts`**
- Admin creates member "Priya" → person row and user row created; "Priya" appears in "Spent by" dropdown on transaction form
- Admin deactivates "Priya" → login blocked; "Priya" still shows on historical transaction
- Admin creates non-login person "Kid" → appears in "Spent for" dropdown

**`transactions.spec.ts`**
- Create income: amount, date, category, account, description → saved, appears in list
- Create expense with all optional fields → saved correctly
- Filter list by date range → only matching transactions shown
- Edit transaction → changes reflected immediately
- Delete transaction → removed from list; budget totals updated

**`itemised-purchase.spec.ts`**
- Add 3 line items to a transaction
- Type "Ground Nut" → autocomplete suggestions appear → select → unit + category auto-filled
- Change quantity → running total updates in real time
- Set unit to "g" when product default is "kg" → conversion hint "Price per 100g" shown
- Total mismatch with transaction amount → warning banner; tick override → save proceeds

**`budget-alerts.spec.ts`**
- Admin creates Groceries budget ₹5,000/month with 80% and 100% thresholds
- Enter ₹4,000 expense → no banner
- Enter ₹1 more → 80% alert banner visible
- Enter ₹1,000 more → 100% alert banner visible
- Dashboard budget bar turns amber, then red

**`recurring.spec.ts`**
- Create monthly recurring electricity bill (variable amount)
- Manually trigger job via test endpoint → pending entry appears in dashboard
- Open pending entry → amount is blank → fill in ₹800 → confirm
- Trigger job again → no second entry (idempotency)

**`reports.spec.ts`**
- Navigate to each of the 11 report types → chart renders, table has rows
- Change date range → data updates
- Download CSV → file downloaded, row count matches table
- Click a category bar → drills down to that category's transactions

**`csv-import.spec.ts`**
- Upload valid CSV → column-mapping step shown → map columns → preview shows rows
- Upload CSV with one duplicate row → duplicate flagged in yellow → user skips it → rest imported
- Upload CSV with invalid rows → errors shown per row; valid rows still importable

**`ocr-receipt.spec.ts`**
- Upload a receipt image (test fixture) → OCR extracts amount, date, merchant
- Extracted fields pre-fill `TransactionForm` → user edits merchant name → saves
- Broken image (empty file) → graceful error message, form not shown

**`email-import.spec.ts`**
- Configure IMAP account (test IMAP server in CI) → test-connection succeeds
- Trigger fetch → Blinkit order appears in review queue with correct line items
- Open queue entry → pre-filled `TransactionForm` → confirm → transaction saved
- Trigger fetch again → same order not re-imported

**`reconciliation.spec.ts`**
- Upload PDF statement for HDFC Credit Card → entries parsed and displayed
- App has matching transaction → auto-suggestion highlighted → user confirms match
- Unmatched bill entry → promote to new transaction → pre-filled, saved
- Mark one entry Ignored → statement reaches Reconciled status when all entries handled

**`settings-hierarchy.spec.ts`**
- Add a new category "Snacks" under Groceries → appears in transaction form category picker
- Rename "Snacks" → updated everywhere
- Add expense transaction tagged to "Snacks" → delete "Snacks" → 409 error, category not deleted
- Reorder siblings via drag → new order persisted on refresh

**`tenant-isolation.spec.ts`**
- Log in as Household A → enter a transaction
- Open new browser context → log in as Household B
- Household B cannot see Household A's transaction in list or by direct URL

---

### 9.8 CI Configuration
...
jobs:
  backend:
    steps:
      - ./gradlew test          # unit + @DataJpaTest + @WebMvcTest (TC spins PG)
      - ./gradlew integrationTest  # @SpringBootTest tests (separate source set)
  ...
  e2e:
    ...
    steps:
      ...
      - npx playwright test     # Playwright specs (headless Chromium)
    if: github.event_name == 'pull_request'
```

TestContainers pulls the `postgres:16` Docker image automatically in CI — no manual DB setup needed for backend tests.

---

## 10. Risk Areas — De-Risk Early
